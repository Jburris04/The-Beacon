(function () {
  ...
  function impact(s) {
    const general=s.why_it_matters?.general||'',providers=s.why_it_matters?.providers||'',patients=s.why_it_matters?.patients||'';if(!general&&!providers&&!patients)return '';
    return `<section class="impact-section"><p class="eyebrow">Why It Matters</p>${general?`<p>${esc(general)}</p>`:''}${providers||patients?`<div class="impact-grid"><article><span>01</span><h2>For Providers</h2><p>${esc(providers)}</p></article><article><span>02</span><h2>For Patients</h2><p>${esc(patients)}</p></article></div>`:''}</section>`;
  }
  ...
  async function renderArticle(){
    ...
    $('#article-body').innerHTML=media+body.map((p,i)=>`${i===1&&s.pull_quote?`<blockquote>${esc(s.pull_quote)}</blockquote>`:''}<p>${esc(p)}</p>`).join('') + (body.length<2&&s.pull_quote?`<blockquote>${esc(s.pull_quote)}</blockquote>`:'') + (s.chain_reaction?`<section class="chain-reaction"><p class="eyebrow">The Chain Reaction</p><p>${esc(s.chain_reaction).replace(/\n/g,'<br>')}</p></section>`:'') + impact(s) + bottomLine(s.bottom_line)+(s.sources?.length?`<section class="sources"><p class="eyebrow">Sources</p>${s.sources.map(x=>`<a href="${esc(x.url)}" target="_blank" rel="noopener">${esc(x.title)} ↗</a>`).join('')}</section>`:'')+(related.length?`<section class="related"><p class="eyebrow">Related reporting</p>${related.map(x=>`<a href="${link(`article/${x.slug}/index.html`)}">${esc(x.headline)} <span>→</span></a>`).join('')}</section>`:'');
  }
